...
week_end_friday <- function(x) {
  weekday <- as.POSIXlt(x)$wday
  as.Date(x + ((5L - weekday) %% 7L))
}
...
cumulative_fx_summary <- function(model, pre_terms, change_terms, hac_lag,
                                  specification, frequency = "daily") {
  vc <- newey_west(model, lag = hac_lag)
  pre_weights <- setNames(rep(1, length(pre_terms)), pre_terms)
  change_weights <- setNames(rep(1, length(change_terms)), change_terms)
  post_weights <- c(pre_weights, change_weights)
  pre <- linear_test(model, pre_weights, null = -1, vcov_matrix = vc)
  change <- linear_test(model, change_weights, null = 0, vcov_matrix = vc)
  post <- linear_test(model, post_weights, null = -1, vcov_matrix = vc)
  ...
  data.frame(
    ...
    hac_lag = hac_lag,
    rows = nobs(model),
    pre_exposure = pre$estimate,
    pre_se = pre$std_error,
    pre_conf_low = pre$estimate - critical * pre$std_error,
    pre_conf_high = pre$estimate + critical * pre$std_error,
    pre_vs_minus1_p = pre$p_value,
    post_exposure = post$estimate,
    post_se = post$std_error,
    post_conf_low = post$estimate - critical * post$std_error,
    post_conf_high = post$estimate + critical * post$std_error,
    post_vs_minus1_p = post$p_value,
    exposure_change = change$estimate,
    change_se = change$std_error,
    change_conf_low = change$estimate - critical * change$std_error,
    change_conf_high = change$estimate + critical * change$std_error,
    change_p = change$p_value,
    row.names = NULL
  )
}
...
ff5$FF_start <- c(as.Date(NA), head(ff5$date, -1L))
mom$MOM_start <- c(as.Date(NA), head(mom$date, -1L))
...
# Simple returns are retained only for a return-definition sensitivity analysis.
# They are exact transformations of the same price ratios, not a second data source.
hewj_ret$HEWJ_simple <- 100 * expm1(hewj_ret$HEWJ_ret / 100)
ewj_ret$EWJ_simple <- 100 * expm1(ewj_ret$EWJ_ret / 100)
fx_ret$JPY_app_simple <- 100 * expm1(fx_ret$JPY_app / 100)
nikkei_ret$Nikkei_simple <- 100 * expm1(nikkei_ret$Nikkei_ret / 100)
vix_ret$dVIX_simple <- 100 * expm1(vix_ret$dlog_VIX / 100)
...
etf_returns$Y_simple <- etf_returns$HEWJ_simple - etf_returns$EWJ_simple
names(etf_returns)[names(etf_returns) == "HEWJ_ret_start"] <- "Y_start"
etf_returns <- etf_returns[, c("date", "Y_start", "Y", "Y_simple")]
...
simple_columns <- c("Y_simple", "JPY_app_simple", "Nikkei_simple", "dVIX_simple")
end_date_aligned <- merged[complete.cases(merged[, c(analysis_columns, simple_columns,
                                                     "FF_start", "MOM_start")]), ]
...
analysis_extended <- end_date_aligned[joint_interval_match,
                                      c(analysis_columns, simple_columns, "Y_start",
                                        "FF_start", "MOM_start")]
analysis_data <- analysis_extended[, analysis_columns]
analysis_data <- analysis_data[order(analysis_data$date), ]
analysis_extended <- analysis_extended[order(analysis_extended$date), ]
stopifnot(nrow(analysis_data) >= 1000L)
stopifnot(nlevels(analysis_data$Post) == 2L)

alignment_audit <- data.frame(
  series = c("JPY_app", "Nikkei_ret", "dlog_VIX", "Japan_FF5", "Japan_MOM"),
  source_clock = c("12:00 America/New_York", "Tokyo market close",
                   "U.S. market close", "Japan trading session", "Japan trading session"),
  native_return_rows = c(nrow(fx_ret), nrow(nikkei_ret), nrow(vix_ret), nrow(ff5), nrow(mom)),
  ...
  exact_interval_start_matches_Y = c(
    ...
    sum(end_date_aligned$FF_start == end_date_aligned$Y_start),
    sum(end_date_aligned$MOM_start == end_date_aligned$Y_start)
  ),
  final_joint_interval_rows = nrow(analysis_data),
  # Only source-level dates are available. Even the VIX "close" is not treated
  # as proof of the ETF's exact adjusted-close timestamp.
  intraday_clock_matches_etf_close = c(FALSE, FALSE, FALSE, FALSE, FALSE)
)
...
factor_exact_match <- joint_interval_match &
  end_date_aligned$Y_start == end_date_aligned$FF_start &
  end_date_aligned$Y_start == end_date_aligned$MOM_start
factor_exact_data <- end_date_aligned[factor_exact_match, analysis_columns]
factor_exact_data <- factor_exact_data[order(factor_exact_data$date), ]
factor_exact_model <- lm(full_formula, data = factor_exact_data)
...
factor_interval_sensitivity <- rbind(
  summarize_fx_model(full_model, "Primary price-clock alignment", analysis_data,
                     "ETF, FX, Nikkei and VIX interval starts match; factor dates match by end date"),
  summarize_fx_model(factor_exact_model, "All return interval starts match", factor_exact_data,
                     "Also requires FF5 and MOM inferred previous trading dates to match Y_start")
)

# FRED DEXJPUS is measured at noon New York while the ETF response is based on
# U.S. closes. A Dimson-style adjacent-return model estimates the cumulative FX
# exposure displaced across date labels by non-synchronous clocks. The lead is
# for explanatory clock reconciliation only and is never used for prediction.
nonsync_data <- analysis_data
nonsync_data$JPY_lag1 <- c(NA_real_, head(nonsync_data$JPY_app, -1L))
nonsync_data$JPY_lead1 <- c(tail(nonsync_data$JPY_app, -1L), NA_real_)
nonsync_data <- nonsync_data[complete.cases(nonsync_data[, c("JPY_lag1", "JPY_lead1")]), ]
nonsync_formula <- Y ~ (JPY_lag1 + JPY_app + JPY_lead1) * Post + Nikkei_ret +
  SMB + HML + RMW + CMA + MOM + dlog_VIX + rate_diff
nonsync_model <- lm(nonsync_formula, data = nonsync_data)

current_pre_terms <- c("JPY_app")
current_change_terms <- c("JPY_app:PostPost")
nonsync_pre_terms <- c("JPY_lag1", "JPY_app", "JPY_lead1")
nonsync_change_terms <- c("JPY_lag1:PostPost", "JPY_app:PostPost", "JPY_lead1:PostPost")

# Weekly endpoints use the last date in each Friday-labelled week on which ETF
# prices and FRED FX are all observed. This does not invent intraday prices; it
# reduces a few-hour clock mismatch relative to the longer return window.
common_levels <- Reduce(function(x, y) merge(x, y, by = "date"),
                        list(hewj[, c("date", "HEWJ")], ewj[, c("date", "EWJ")],
                             fx[, c("date", "DEXJPUS")]))
common_levels <- common_levels[complete.cases(common_levels), ]
common_levels$week_end <- week_end_friday(common_levels$date)
weekly_endpoints <- do.call(rbind, lapply(split(common_levels, common_levels$week_end), function(block) {
  block[which.max(block$date), c("date", "HEWJ", "EWJ", "DEXJPUS")]
}))
weekly_endpoints <- weekly_endpoints[order(weekly_endpoints$date), ]
weekly_data <- data.frame(
  date = weekly_endpoints$date[-1L],
  interval_start = weekly_endpoints$date[-nrow(weekly_endpoints)],
  Y = 100 * diff(log(weekly_endpoints$HEWJ / weekly_endpoints$EWJ)),
  JPY_app = -100 * diff(log(weekly_endpoints$DEXJPUS))
)
break_date <- as.Date("2020-03-11")
weekly_data$Post <- ifelse(weekly_data$date < break_date, "Pre",
                           ifelse(weekly_data$interval_start > break_date, "Post", NA_character_))
weekly_data <- weekly_data[!is.na(weekly_data$Post), ]
weekly_data$Post <- factor(weekly_data$Post, levels = c("Pre", "Post"))
weekly_model <- lm(Y ~ JPY_app * Post, data = weekly_data)

nonsynchronous_fx_sensitivity <- rbind(
  cumulative_fx_summary(full_model, current_pre_terms, current_change_terms, 5L,
                        "Current-date full model", "daily"),
  cumulative_fx_summary(full_model, current_pre_terms, current_change_terms, 22L,
                        "Current-date full model", "daily"),
  cumulative_fx_summary(nonsync_model, nonsync_pre_terms, nonsync_change_terms, 5L,
                        "Dimson adjacent-return full model", "daily"),
  cumulative_fx_summary(nonsync_model, nonsync_pre_terms, nonsync_change_terms, 22L,
                        "Dimson adjacent-return full model", "daily"),
  cumulative_fx_summary(weekly_model, current_pre_terms, current_change_terms, 4L,
                        "Last-common-date weekly FX model", "weekly")
)
...
# windows select the predictive model; the 2024-01-24 onward period is excluded
# from fitting and model selection, then used for final chronological evaluation.
...
static_fx_model <- lm(Y ~ JPY_app, data = development)
static_fx_prediction <- as.numeric(predict(static_fx_model, newdata = test))
theoretical_minus_one_prediction <- -test$JPY_app
...
test_metrics <- do.call(rbind, lapply(list(
  ...
  static_fx = list(name = "Static FX regression benchmark", pred = static_fx_prediction),
  theoretical = list(name = "Theoretical minus-one spot benchmark",
                     pred = theoretical_minus_one_prediction),
...
test_predictions <- data.frame(
  ...
  static_fx = static_fx_prediction,
  theoretical_minus_one = theoretical_minus_one_prediction,
...
split_summary <- do.call(rbind, lapply(levels(analysis_data$split), function(split_name) {
  ...
  data.frame(split = split_name, rows = nrow(block), start_date = min(block$date), end_date = max(block$date),
             purpose = switch(split_name,
                              ...
                              test = "chronological evaluation held out from model selection"))
}))

# Log returns are exact continuously compounded price-ratio changes. To measure
# whether their compression of large daily moves matters in this dataset, rerun
# the identical split and candidate-selection rule with arithmetic returns.
simple_data <- data.frame(
  date = analysis_extended$date,
  Y = analysis_extended$Y_simple,
  JPY_app = analysis_extended$JPY_app_simple,
  Post = analysis_extended$Post,
  Nikkei_ret = analysis_extended$Nikkei_simple,
  SMB = analysis_extended$SMB,
  HML = analysis_extended$HML,
  RMW = analysis_extended$RMW,
  CMA = analysis_extended$CMA,
  MOM = analysis_extended$MOM,
  dlog_VIX = analysis_extended$dVIX_simple,
  rate_diff = analysis_extended$rate_diff
)
simple_development <- simple_data[simple_data$date < test_start_date, ]
simple_test <- simple_data[simple_data$date >= test_start_date, ]
simple_rolling_origin_fold_metrics <- do.call(rbind, lapply(names(candidate_formulas), function(model_name) {
  do.call(rbind, lapply(seq_len(nrow(rolling_origin_folds)), function(i) {
    fold_spec <- rolling_origin_folds[i, ]
    fold_train <- simple_development[simple_development$date <= fold_spec$train_end, ]
    fold_validation <- simple_development[
      simple_development$date >= fold_spec$validation_start &
        simple_development$date <= fold_spec$validation_end, ]
    fitted_candidate <- lm(candidate_formulas[[model_name]], data = fold_train)
    metrics <- prediction_metrics(fold_validation$Y,
                                  predict(fitted_candidate, newdata = fold_validation))
    data.frame(model = model_name, fold = fold_spec$fold,
               RMSE = unname(metrics["RMSE"]), MAE = unname(metrics["MAE"]))
  }))
}))
simple_model_selection <- do.call(rbind, lapply(names(candidate_formulas), function(model_name) {
  rows <- simple_rolling_origin_fold_metrics[
    simple_rolling_origin_fold_metrics$model == model_name, ]
  data.frame(model = model_name, mean_RMSE = mean(rows$RMSE), mean_MAE = mean(rows$MAE))
}))
simple_selected_model_name <- simple_model_selection$model[
  which.min(simple_model_selection$mean_RMSE)]
simple_selected_model <- lm(candidate_formulas[[simple_selected_model_name]],
                            data = simple_development)
simple_test_prediction <- as.numeric(predict(simple_selected_model, newdata = simple_test))
simple_test_score <- prediction_metrics(simple_test$Y, simple_test_prediction)
simple_full_model <- lm(full_formula, data = simple_data)
simple_full_summary <- summarize_fx_model(simple_full_model, "Arithmetic returns", simple_data,
                                          "Same observations and model workflow as log returns")
log_full_summary <- summarize_fx_model(full_model, "Log returns", analysis_data,
                                       "Primary continuously compounded definition")

return_definition_sensitivity <- data.frame(
  definition = c("Log returns", "Arithmetic returns"),
  selected_model = c(selected_model_name, simple_selected_model_name),
  rolling_mean_RMSE = c(
    model_selection_rolling_origin$mean_RMSE[
      model_selection_rolling_origin$model == selected_model_name],
    simple_model_selection$mean_RMSE[
      simple_model_selection$model == simple_selected_model_name]
  ),
  test_response_sd = c(sd(test$Y), sd(simple_test$Y)),
  test_abs_q95 = c(unname(quantile(abs(test$Y), .95)),
                   unname(quantile(abs(simple_test$Y), .95))),
  test_abs_max = c(max(abs(test$Y)), max(abs(simple_test$Y))),
  selected_test_RMSE = c(test_metrics$RMSE[1], unname(simple_test_score["RMSE"])),
  selected_test_MAE = c(test_metrics$MAE[1], unname(simple_test_score["MAE"])),
  pre_slope = c(log_full_summary$pre_slope, simple_full_summary$pre_slope),
  interaction = c(log_full_summary$interaction, simple_full_summary$interaction),
  interaction_hac5_p = c(log_full_summary$interaction_hac5_p,
                         simple_full_summary$interaction_hac5_p),
  post_slope = c(log_full_summary$post_slope, simple_full_summary$post_slope)
)
...
write.csv(factor_interval_sensitivity,
          file.path(robustness_results_dir, "factor_interval_sensitivity.csv"), row.names = FALSE)
write.csv(nonsynchronous_fx_sensitivity,
          file.path(robustness_results_dir, "nonsynchronous_fx_sensitivity.csv"), row.names = FALSE)
write.csv(return_definition_sensitivity,
          file.path(robustness_results_dir, "return_definition_sensitivity.csv"), row.names = FALSE)
...
saveRDS(list(full_model = full_model, baseline_model = baseline_model, hac5 = vcov_hac5,
             ...
             nonsync_model = nonsync_model, weekly_model = weekly_model,
             simple_full_model = simple_full_model,
...
plot(analysis_data$date, analysis_data$Y, pch = 16, cex = .35,
     ...
     main = "Development period and held-out-from-selection evaluation")
abline(v = as.numeric(max(development$date)), lty = 2, col = "grey35")
legend("topright", legend = c("development", "held out from selection"),
       col = split_cols, pch = 16, bty = "n")
dev.off()
...
axis(2, at = 1:4, labels = c(rolling_origin_folds$fold, "Final evaluation"), las = 1)
...
legend("bottomright", legend = c("Expanding train", "Validation", "Held out from selection"),
...
png(file.path(diagnostics_figures_dir, "time_and_return_definition_sensitivity.png"),
    width = 1900, height = 1050, res = 180)
par(mfrow = c(1, 2), mar = c(5.5, 4.5, 3.2, 1.2))
clock_rows <- nonsynchronous_fx_sensitivity[c(1, 4, 5), ]
clock_labels <- c("Daily current\nHAC(5)", "Daily Dimson\nHAC(22)", "Weekly common\nHAC(4)")
x_positions <- seq_len(nrow(clock_rows))
plot(x_positions - .08, clock_rows$pre_exposure, pch = 19, col = "#1f6aa5",
     xlim = c(.55, 3.45), ylim = range(c(clock_rows$pre_conf_low, clock_rows$pre_conf_high,
                                         clock_rows$post_conf_low, clock_rows$post_conf_high, -1)),
     xaxt = "n", xlab = "Clock treatment", ylab = "Cumulative JPY exposure",
     main = "Clock-alignment sensitivity")
segments(x_positions - .08, clock_rows$pre_conf_low,
         x_positions - .08, clock_rows$pre_conf_high, col = "#1f6aa5", lwd = 2)
points(x_positions + .08, clock_rows$post_exposure, pch = 17, col = "#d94b36")
segments(x_positions + .08, clock_rows$post_conf_low,
         x_positions + .08, clock_rows$post_conf_high, col = "#d94b36", lwd = 2)
axis(1, at = x_positions, labels = clock_labels)
abline(h = -1, lty = 2, col = "grey40")
legend("bottomright", legend = c("Pre", "Post", "Minus-one reference"),
       col = c("#1f6aa5", "#d94b36", "grey40"), pch = c(19, 17, NA),
       lty = c(NA, NA, 2), bty = "n")

metric_matrix <- rbind(return_definition_sensitivity$test_response_sd,
                       return_definition_sensitivity$selected_test_RMSE)
barplot(metric_matrix, beside = TRUE, names.arg = c("Log", "Arithmetic"),
        col = c("#1f6aa5", "#e69f00"), ylab = "Percentage points",
        main = "Does log scaling suppress test variation?")
legend("topright", legend = c("Test response SD", "Selected-model RMSE"),
       fill = c("#1f6aa5", "#e69f00"), bty = "n")
dev.off()
...
log_lines <- c(
  ...
  "",
  "Final period held out from fitting and model selection:",
  paste(capture.output(print(test_metrics, row.names = FALSE, digits = 7)), collapse = "\n"),
  "",
  "Non-synchronous clock sensitivity:",
  paste(capture.output(print(nonsynchronous_fx_sensitivity, row.names = FALSE, digits = 7)),
        collapse = "\n"),
  "",
  "Factor-interval sensitivity:",
  paste(capture.output(print(factor_interval_sensitivity, row.names = FALSE, digits = 7)),
        collapse = "\n"),
  "",
  "Log-versus-arithmetic return sensitivity:",
  paste(capture.output(print(return_definition_sensitivity, row.names = FALSE, digits = 7)),
        collapse = "\n"),
  "",
